...
import type {MeshInitializer} from "./dual-mesh/index.ts";
import {choosePoints} from "./generate-points.ts";
import type {Mesh} from "./types.d.ts";

export async function makeMesh(seed?: number, spacing?: number) {
    const _seed = seed ?? param.mesh.seed;
    const _spacing = spacing ?? param.spacing;
    const _mountainSpacing = param.mountainSpacing;

    let {points, numExteriorBoundaryPoints, numInteriorBoundaryPoints, numMountainPoints} =
        choosePoints(_seed, _spacing, _mountainSpacing);
