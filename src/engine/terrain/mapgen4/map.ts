...
function calculateMountainDistance(mesh: Mesh, t_peaks: number[], spacing: number, jaggedness: number, randFloat: () => number, distance_t: Float32Array) {
    distance_t.fill(-1);
    // Initialize all peak triangles to distance 0 before BFS so they are not
    // overwritten by expansion from neighboring peaks (multi-source BFS must
    // seed all roots first).
    for (let t of t_peaks) {
        distance_t[t] = 0;
    }
